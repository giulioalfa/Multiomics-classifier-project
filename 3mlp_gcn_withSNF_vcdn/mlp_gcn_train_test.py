...
def prepare_trte_data(data_folder, view_list):
    ...
    print("early_agg", early_aggregation_all.shape)
    print("snf computed")
...
def gen_trte_adj_mat(fused_snf_train, fused_snf_test):
    ...
    print("adj train shape", adj_train_list[0].shape)
    print("adj test shape", adj_test_list[0].shape)
...
def train_epoch(data_list, adj_list, label, one_hot_label, sample_weight, model_dict, optim_dict, train_VCDN=True):
    ...
    for i in range(num_view):
      if i == 3: 
          ...
          optim_dict["N{:}".format(i+1)].zero_grad()
          ci_loss = 0
          ci = model_dict["N{:}".format(i+1)](data_list[i])
          
          ci_loss = torch.mean(torch.mul(criterion(ci, label),sample_weight))
          
          ci_loss.backward()
          optim_dict["N{:}".format(i+1)].step()
          loss_dict["N{:}".format(i+1)] = ci_loss.detach().cpu().numpy().item()
    if train_VCDN and num_view >= 2:
        ...
        for i in range(num_view):
          if i==3:
            ...
            ci_list.append(model_dict["N{:}".format(i+1)](data_list[i]))
...
def train_test(data_folder, view_list, num_class,
    ...
    print("data tr shape :", data_tr_list[0].shape,data_tr_list[1].shape, data_tr_list[2].shape, data_tr_list[3].shape )
